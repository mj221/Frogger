...
void init_button_interrupts(void) {
	...
	cli();
	// disable the interrupt (see datasheet page 91)
	PCICR |= (1<<PCIE1);
...
int8_t button_repeat(void) {
	...
	int8_t return_value = NO_BUTTON_PUSHED;
	
	uint8_t button_state = PINB & 0x0F;

	if (get_current_time() > button_repeat_time && button_state == last_button_state) {
		if (button_state == 1){
			button_repeat_time = get_current_time() + REPEAT_DELAY;
			return_value = 0;
...
ISR(PCINT1_vect) {
	...
	for(uint8_t pin=0; pin<=3; pin++) {
		if(queue_length < BUTTON_QUEUE_SIZE && 
			...
			button_repeat_time = get_current_time() + INIT_DELAY;
		}
